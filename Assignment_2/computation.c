...
void compute(double complex x0,double complex *root, short int *id, int *iterations);
...
int main() {
  ...
  for ( size_t ix = 0, jx = 0; ix < MAX_LINES; ++ix, jx+=MAX_LINES )
	...
	// creation of initial points' matrix
...
void compute(double complex x0,double complex *root, short int *id, int *iter){
	...
	while(conv == -1) {
		...
		for (size_t ix = 0; ix < d; ix++) {
			...
			else if(creal(x1) >= n_root || creal(x1) <= -n_root || cimag(x1) >= n_root || cimag(x1) <= -n_root){ //trying not to use cabs()
				//printf("special case x1 >= 10000000000\n");
				*id = 10; // 10 would be the value for when newtons method tends to infinity
				conv = 1;
				break;
			}
